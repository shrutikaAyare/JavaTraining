...
public class SelectorDemo {
    ...
    for(;;) {
        ...
        while(itr.hasNext()) {
            ...
            if(k.isAcceptable()) {
            	//first tym this part will execute once the channel is acceptable then it goes to readable
                ...
                if(k.isReadable()){
                    ...
                    //allocates memory at server side
                    ByteBuffer buffer = ByteBuffer.allocate(256); //it is datatype-->allocates the byte into buffer--study nio api
                    client.read(buffer);
